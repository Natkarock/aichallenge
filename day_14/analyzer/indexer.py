...
class ProjectIndexer:
    ...
    def _skip_by_name(self, p: Path) -> str | None:
        ...
        if self.skip_sourcemaps and ext == ".map":
            return "sourcemap"
        if self.skip_minified and (name.endswith(".min.js") or name.endswith(".min.css")):
            return "minified"
        return None
            ...
            for fp in all_files:
                ...
                try:
                    if is_probably_text(fp):
                        raw = fast_head_text(fp, limit_chars=self.max_file_chars, time_budget_s=per_file_budget)
                        file_chunks = safe_chunk_text(raw, max_chars=max_chars, overlap=overlap, max_chunks=self.max_chunks_per_file)
                        if not file_chunks:
                            file_chunks = [""]
                        for i, ch in enumerate(file_chunks):
                            chunks.append(DocChunk(path=str(fp), idx=i, text=ch))
                    else:
                        chunks.append(DocChunk(path=str(fp), idx=0, text=binary_meta_chunk(fp)))
                except TimeoutError:
