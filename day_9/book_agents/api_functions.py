...
import os
from .const import API_URL, MCP_SERVER_URL, MCP_PROXY_AUTH_TOKEN, MCP_AUTH_HEADER_NAME
...
def build_openlibrary_mcp_tool(allowed_tools: List[str] = None) -> Dict[str, Any]:
    allowed_tools = allowed_tools or [
        'get_book_by_title', 'get_book_cover', 'get_book_by_id',
        'get_authors_by_name', 'get_author_info', 'get_author_photo'
    ]
    headers = {}
    if MCP_PROXY_AUTH_TOKEN:
        headers[MCP_AUTH_HEADER_NAME] = f'Bearer {MCP_PROXY_AUTH_TOKEN}'
        headers['Authorization'] = f'Bearer {MCP_PROXY_AUTH_TOKEN}'
        headers['MCP-Proxy-Auth-Token'] = MCP_PROXY_AUTH_TOKEN
        headers['X-MCP-Proxy-Auth-Token'] = MCP_PROXY_AUTH_TOKEN
    # return {
    #     'type': 'mcp',
    #     'server_label': 'openlibrary',
