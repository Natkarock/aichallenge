...
from .api_functions import _post, _extract_text, _get_usage, rough_token_estimate, tokens_to_chars, estimate_messages_tokens
...
class Agent3SummarizerGUI:
    ...
    def _summarize_map_reduce(self, raw_text: str, target_budget_tokens: int) -> Tuple[str, Dict[str, int]]:
        ...
        system = (
            "Ты — редактор. Объедини тезисы из всех кусков в цельное краткое саммари "
            "(8–12 bullets + 5–10 тегов). Без повтора одно и того же."
        )
        payload = {
            "model": self.model,
            "input": [
